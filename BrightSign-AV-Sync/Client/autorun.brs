...
' This script syncs multiple BrightSigns (Audio and Video) by sending messages over ethernet.
' Based on Zach Poff, modified by Erik Anton Reinhardt.
...
ClientIP = "192.168.1." + Str(10 + ClientNumber).Trim()
...
listen:
	...
	elseif type(msg) = "roVideoEvent" then
		...
		if eventType = 8 then ' Video finished
			print "Video ended - forcing black screen"
			v.Stop()
			v = invalid
			sleep(100)
			' Recreate video player for black screen
			v = CreateObject("roVideoPlayer")
			v.SetPort(p)
			v.SetViewMode(ScaleMode)
			v.SetVolume(audioVolume)
			print "Player reset - black screen active"
		endif
	endif
